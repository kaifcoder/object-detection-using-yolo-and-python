...
def run(
        ...
        limitpeople=10000,  # maximum detections per image
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            s += '%gx%g ' % im.shape[2:]
            displayCount = ''  # print string
            # vehicles = 0  # print string
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    # print(names[int(c)])  # add to string
                    displayCount += f"{names[int(c)]}->{n},"
            ...
            if view_img:
                cv2.rectangle(
                    im0, (0, 0), (int(im0.shape[1] * 100), 60), (0, 0, 0), -1)
                cv2.putText(im0, f"{displayCount}",
                            (5, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
                if limitpeople == 10000:
                    cv2.putText(im0, f"People Limit: None",
                                (5, 55), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
                else:
                    cv2.putText(im0, f"People Limit: {limitpeople}",
                                (5, 55), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
                if "person" in displayCount:
                    if n > limitpeople:
                        cv2.putText(im0, 'Limit of People Exceeded',
                                    (int(im0.shape[1] * 0.60), 55), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 1)
...
def parse_opt():
    ...
    parser.add_argument('--conf-thres', type=float,
                        default=0.35, help='confidence threshold')
    parser.add_argument('--iou-thres', type=float,
                        default=0.45, help='NMS IoU threshold')
    parser.add_argument('--max-det', type=int, default=10000,
                        help='maximum detections per image')
    parser.add_argument('--limitpeople', type=int,
                        default=10000, help='maximum people limit per image')
